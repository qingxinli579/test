<template>
  <div id="wangeditor" style="width:50%">
    <el-button
      size="small"
      @click="changeFun()"
    >切换至{{ !textareaShow ? "html源码" : "富文本" }}</el-button>
    <div v-show="!textareaShow" ref="editorElem" @keydown="keyFUN" />
    <el-input
      v-show="textareaShow"
...
export default {
  ...
  methods: {
    ...
    filterFUN(url, alt) {
      ...
      this.emojiList.forEach(element => {
        const url =
          ...
          element.alt +
          '" data-w-e="1">'
        const url2 = '<img src="' + element.src + '" data-w-e="1">'
    ...
    filterFUNX() {
      this.emojiList.forEach(element => {
        const url = '<img src="' + element.src + '" data-w-e="1">'
